    ...
    while next_sibling.at("tr[#{tr_count}]/td[1]")
      ...
      unless oldpd = ProcessDocument.find_by_external_link(process_document.external_link)
        ...
        if process_document.external_type.index("lög") # New changed law
          #FFJ document = LawDocumentElement.create_elements(process_document, process_document.priority_process_id, process_document.id, process_document.external_link,process_type)
          ...
          #FFJ document = ChangeDocumentElement.create_elements(process_document, process_document.priority_process_id, process_document.id, process_document.external_link,process_type)
        end         
        if document
